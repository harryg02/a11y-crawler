...
async function main(): Promise<number> {
  ...
  try {
    // Exit non-zero when the crawl was cut short, so scanManager records it as
    // interrupted-and-resumable instead of completed.
    const { aborted } = await runCrawl({ chromium });
    return aborted ? 1 : 0;
  } finally {
    clearTimeout(budget);
  }
