...
const SEVERITY_STYLE: Record<string, string> = {
  ...
  serious:  'text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-950 border-orange-200 dark:border-orange-800',
  moderate: 'text-yellow-700 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-950 border-yellow-200 dark:border-yellow-800',
  minor:    'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800',
};
  ...
  return (
    ...
    <div className="max-w-220 mx-auto p-8">
          ...
          {filters.map(f => (
            <button
              ...
              className={`
                ...
                focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-white focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-gray-950
                ${filter === f.id
                  ? 'bg-gray-200 border-gray-500 text-gray-900 dark:bg-gray-700 dark:border-gray-500 dark:text-white'
                  : 'bg-gray-100 border-gray-300 text-gray-600 hover:border-gray-500 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-400 dark:hover:border-gray-400'}
              `}
            >
              {f.label}
            </button>
      ...
      {filtered.length > 0 ? (
        ...
        <p className="text-base text-gray-600 dark:text-gray-400 text-center mt-8">No violations for this filter.</p>
      )}
    </div>
    </div>
  );
...
function ViolationCard({ violation, defaultExpanded = false }: { violation: Violation; defaultExpanded?: boolean }) {
  ...
  return (
    <article className="bg-gray-200 dark:bg-gray-800 border-2 border-gray-400 dark:border-gray-600 rounded-md overflow-hidden">
      {/* Collapsed header — always visible, click to toggle */}
      <button
        type="button"
        onClick={() => setExpanded(e => !e)}
        aria-expanded={expanded}
        className="w-full flex items-center gap-3 px-4 py-3.5 text-left hover:bg-gray-750 transition-colors focus:outline-none focus:ring-2 focus:ring-inset focus:ring-gray-900 dark:focus:ring-white"
      >
        <span
        ...
        <span className="flex-1 text-base font-medium text-gray-900 dark:text-white leading-snug">{violation.help}</span>
        <span className="text-base text-gray-600 dark:text-gray-400 shrink-0">{total}</span>
        {expanded
          ? <ChevronDown size={18} className="text-gray-600 dark:text-gray-400 shrink-0" aria-hidden="true" />
          : <ChevronRight size={18} className="text-gray-600 dark:text-gray-400 shrink-0" aria-hidden="true" />
        }
      </button>

      {/* Expanded body */}
      {expanded && (
        <div className="border-t border-gray-300 dark:border-gray-700 px-4 py-4 space-y-4">
          ...
          {total > 1 && (
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="w-11 h-11 flex items-center justify-center rounded-md bg-gray-200 border-2 border-gray-400 text-gray-900 hover:bg-gray-300 hover:border-gray-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:bg-gray-600 dark:hover:border-gray-500 disabled:opacity-40 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-200 dark:focus:ring-offset-gray-800 transition-colors"
              >
              ...
              <span className="px-3 text-base text-gray-900 dark:text-white min-w-20 text-center tabular-nums" aria-live="polite" aria-atomic="true">
          ...
          <div className="pt-2 border-t border-gray-300 dark:border-gray-700">
            <a
              ...
              className="inline-flex items-center gap-1.5 text-base text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-200 dark:focus:ring-offset-gray-800 rounded"
            >
              Learn more
